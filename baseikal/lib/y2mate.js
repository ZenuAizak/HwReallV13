...
async function yt(url, quality, type, bitrate, server = 'en68') {
  ...
  switch (type) {
    ...
    case 'mp3':
      list = {
        '1288kbps': table.querySelector('td > a[href="#"]').parentElement.nextSibling.nextSibling.innerHTML
      }
      break
    default:
      list = {}
...
module.exports = {
   ...
   * @param {String} server (avaiable: `id4`, `en60`, `en61`, `en68`)
   */
  yta(url, resol = '1288kbps', server = 'en154') { return yt(url, resol, 'mp3', resol.endsWith('kbps') ? resol.replace(/kbps/g, '') : resol, server) },
  /**
